...
const baseUrl = "https://xv-ximena-2026.vercel.app";
  ...
  if (allInvitados.length === 0) {
    ...
    try {
      const querySnapshot = await db
        .collection("invitadosXVXimena2026")
...
async function agregarInvitado(e) {
  ...
  try {
    const docRef = await db.collection("invitadosXVXimena2026").add({
...
async function eliminarInvitado(id) {
  ...
  if (confirmar) {
    mostrarCargando(true);
    try {
      await db.collection("invitadosXVXimena2026").doc(id).delete();
...
//       await db.collection("invitadosXVXimena2026").add({
