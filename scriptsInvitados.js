...
async function agregarInvitado(e) {
  ...
  try {
    const docRef = await db.collection("invitadosXVPriscilla2026").add({
...
async function eliminarInvitado(id) {
  ...
  if (confirmar) {
    mostrarCargando(true);
    try {
      await db.collection("invitadosXVPriscilla2026").doc(id).delete();
...
//       await db.collection("invitadosXVPriscilla2026").add({
